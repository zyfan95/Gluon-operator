...
namespace Chroma 
{
     ...
     */
    multi4d<LatticeColorMatrix> fun_Operator_part(int len, int dir, multi2d<LatticeColorMatrix> F, LatticeColorMatrix u)
    {
        LatticeColorMatrix un;
        multi2d<LatticeColorMatrix> Fn;
        Fn.resize(Nd,Nd);
        ...
        multi4d<LatticeColorMatrix> Opcomb;
        Opcomb.resize(Nd,Nd,Nd,Nd);
                ...
                for(int i=0;i<4; i++)
                {
                        for(int j=0;j<i;j++)
                        {
                                for(int m=0;m<4; m++)
                                {
                                        for(int n=0;n<m;n++)
                                        {
                                                Opcomb[i][j][m][n] = F[i][j]*un*F[m][n]*adj(un);
                                        }
                                }
                        }
                }
 ...
 *           * The outputs are the 6 operators.
 *                */
    multi1d<LatticeColorMatrix> fun_Operator_inverse(int len, int dir, multi2d<LatticeColorMatrix> F, LatticeColorMatrix u)
    {
        LatticeColorMatrix un;
        multi2d<LatticeColorMatrix> Fn;
        Fn.resize(Nd,Nd);
        ...
        else //non-local
        {
                ...
                for(int i=0;i<4; i++)
                {
                        ...
                        Op[0] += Fn[3][i]*adj(un)*F[3][i]*un;
                        Op[1] += Fn[dir][i]*adj(un)*F[3][i]*un;
                        Op[2] += Fn[dir][i]*adj(un)*F[dir][i]*un;
                        Op[3] += Fn[dir][i]*adj(un)*F[dir][i]*un;
                }
                for(int i=0;i<4; i++)
                {
                        for(int j=0;j<i;j++)
                ...
                for(int i=0;i<4; i++)
                {
                        if(i!=dir && i!=3)
                        {
                                Op[4] += Fn[3][i]*adj(un)*F[dir][i]*un;
                                Op[5] += Fn[dir][i]*adj(un)*F[3][i]*un;
                                Op[6] += Fn[3][i]*adj(un)*F[3][i]*un;
                                Op[7] += Fn[dir][i]*adj(un)*F[dir][i]*un;
                        }
                }

                if(dir==0)
                {
                        Op[8] = Fn[3][2]*adj(un)*F[3][1]*un-Fn[3][1]*adj(un)*F[3][2]*un;
                        Op[9] = Fn[dir][2]*adj(un)*F[dir][1]*un-Fn[dir][1]*adj(un)*F[dir][2]*un;
                        Op[10] = Fn[dir][2]*adj(un)*F[3][1]*un-Fn[dir][1]*adj(un)*F[3][2]*un;
                }
                else if(dir==1)
                {
                        Op[8] = Fn[3][0]*adj(un)*F[3][2]*un-Fn[3][2]*adj(un)*F[3][0]*un;
                        Op[9] = Fn[dir][0]*adj(un)*F[dir][2]*un-Fn[dir][2]*adj(un)*F[dir][0]*un;
                        Op[10] = Fn[dir][0]*adj(un)*F[3][2]*un-Fn[dir][2]*adj(un)*F[3][0]*un;
                }
                else if(dir==2)
                {
                        Op[8] = Fn[3][1]*adj(un)*F[3][0]*un-Fn[3][0]*adj(un)*F[3][1]*un;
                        Op[9] = Fn[dir][1]*adj(un)*F[dir][0]*un-Fn[dir][0]*adj(un)*F[dir][1]*un;
                        Op[10] = Fn[dir][1]*adj(un)*F[3][0]*un-Fn[dir][0]*adj(un)*F[3][1]*un;
                }
...
*/
		...
		for(int len = 0; len < mn; len++)
			...
			Op = fun_Operator(len, dir, F, u[dir]); //Obtain the operators
                        ...
                        Opcomb.resize(Nd,Nd,Nd,Nd);
                        Opcomb = 0;
                        Opcomb = fun_Operator_part(len, dir, F, u[dir]);
			...
			for(int t = 0; t < Layout::lattSize()[3]; t++)
				...
				op = 0;
                                ...
                                multi4d<Double> opcomb;
                                opcomb.resize(Nd,Nd,Nd,Nd);
                                opcomb = 0;

                                multi4d<Double> opcombi;
                                opcombi.resize(Nd,Nd,Nd,Nd);
                                opcombi = 0;
                				...
                				for(int i=0;i<4; i++)
                				{
                        			for(int j=0;j<i;j++)
                        			{
                                		for(int m=0;m<4; m++)
                                		{
                                        	for(int n=0;n<m;n++)
                                        	{
                                                        opcomb[i][j][m][n] += real(trace(peekSite(Opcomb[i][j][m][n], tCoords)));
                                                        opcombi[i][j][m][n] += imag(trace(peekSite(Opcomb[i][j][m][n], tCoords)));
                                        	}
                                		}
                        			}
                				}
				...
				/*
                		for(int l = 0; l < 4; l++)
                		{
                                ...
                                for(int n=0;n<m;n++)
                                {
					 QDPIO::cout <<"Opcomb   "<< dir << "  " << len <<"  "<< t <<"  "<< i <<"  "<< j <<"  "<< m <<"  "<< n <<"  "<< opcomb[i][j][m][n]<<std::endl;
                                }
                                }
                                }
                                }
